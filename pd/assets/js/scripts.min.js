...
var GoogleMap = (function () {
    ...
    // _const.mapKey = 'AIzaSyDslI9NouA-sv9_4G9qmruvlBlwzcQFdcY'; // BODO key
    _const.mapKey = 'AIzaSyB7mF-OcvGUnjgd1tr7rg_dWM73IQGHDjk';
    ...
    GoogleMap.loadGoogleMap = function (callback) {
        if (!GoogleMap.mapLoaded) {
            ...
            $.cachedScript('https://maps.googleapis.com/maps/api/js?language=' + (g.v.lang.length ? 'uk' : 'ru') + '&sensor=false&key=' + _const.mapKey)
                .done(function () {
                    // map loaded
...
(function ($) {
  ...
  $(function () {
    ...
    if (windowWidth < laptop) {
      ...
      if ($('.product__additional-head').length && !$('.product__additional-head--wrap').length) {
        $('.product__images').prepend($('<div class="product__additional-head--wrap up-animate"></div>'));
        $('.product__additional-head--wrap').prepend($('.product__additional-head'));
      }
    }

    if (windowWidth < largeMobDesktop) {
      if ($('.product-main').length) {
        $('.product-main .products__buttons').removeClass('item-animate');
        $('.product__additional-head--wrap').append($('.product-main .products__buttons'));
      }
    }

    if (windowWidth < mobileDesktop) {
      if ($('.product__instalments-wrapper').length) {
        $('.product__additional').prepend($('.product__instalments-wrapper'));
      }

      if ($('.maincontent__faq .help').length) {
        $('.faq-main__col').append($('.maincontent__faq .help'));
      }
    }
    ...
    $(window).on('resize', function () {
      ...
      if (windowWidth < laptop) {
        if ($('.product__additional-head').length && !$('.product__additional-head--wrap').length) {
          $('.product__images').prepend($('<div class="product__additional-head--wrap"></div>'));
          $('.product__additional-head--wrap').prepend($('.product__additional-head'));
        }
      }

      if (windowWidth < largeMobDesktop) {
        if ($('.product__additional-head--wrap').length) {
          $('.product__additional-head--wrap').append($('.product-main .products__buttons'));
        }
      }
    ...
    } // selections__slider init


    if ($('.selections__slider').length) {
    ...
    if ($('.checkout__user-tabs').length) {
      $('.checkout__user-tabs').minimalTabs();
    }

    if ($('.faq__tabs').length) {
      $('.faq__tabs').minimalTabs();
    }

    if ($('.addresses__tabs').length) {
      $('.addresses__tabs').minimalTabs();
    } // tooltip init
    ...
    $('.header__auth-link').on('click', function (e) {
      ...
      if (windowWidth > largeMobDesktop) {
        $('html').addClass('__show-overlay __transparent __show-header');
      }
    });
    $('[data="authorization-close"]').on('click', function () {
      $('html').removeClass('__show-overlay __transparent __show-header');
      if ($('.authorization').hasClass('__active')) $('.authorization').removeClass('__active');
    }); // input mask
    ...
    if ($('.reviews-slider').length) {
      ...
      var reviewsSlider = new Swiper($slider, {
        loop: true,
        slidesPerView: 3,
        speed: 600,
        navigation: {
    ...
    $('body').on('click', '.toggle-switch__main', $.debounce(function (e) {
      ...
      $('.product-main .item-animate').removeAttr('style');
    }, 300));
    $('body').on('click', '.toggle-switch button', function () {
      ...
      if ($(this).data('view') == 'interior') {
        $('.item-out').each(function (index) {
          (function (that, i) {
            var t = setTimeout(function () {
              $(that).removeClass('fadeInRight fadeInLeft').addClass('fadeOutRight');
            }, 100 * i);
          })(this, index);
        });
        setTimeout(function () {
          $('.product__interior-nav-wrapper').addClass('active');
        }, 100);
      } else {
        $('.item-out').each(function (index) {
          (function (that, i) {
            var t = setTimeout(function () {
              $(that).removeClass('fadeOutRight fadeOutLeft').addClass('fadeInRight');
            }, 100 * i);
          })(this, index);
        });
        $('.product__interior-nav-wrapper').removeClass('active');
      }
    });
      ...
      if (windowWidth < mobileDesktop) {
        $(window).scroll(function () {
          if ($(this).scrollTop() >= 160) {
            $('.filter-trigger').addClass("sticky");
          } else {
            $('.filter-trigger').removeClass("sticky");
          }
        });
      }
    } // animate
    ...
    $('body').on('click', '.js-btn-continue', function (e) {
      e.preventDefault();
      $(this).toggleClass('hidden');
      $('.btn-buy').toggleClass('hidden');
      $('.checkout__user').removeClass('checkout__steps-item--active');
      $('.checkout__user').find('.checkout__steps-item--content').slideUp(300);
      $('.checkout__order-info').addClass('checkout__steps-item--active').slideDown(300);
    });
    $('body').on('click', '.js-step-btn', function (e) {
      e.preventDefault();
      $('.btn-continue').removeClass('hidden');
      $('.btn-buy').toggleClass('hidden');
      $('.checkout__user').addClass('checkout__steps-item--active');
      $('.checkout__user').find('.checkout__steps-item--content').slideDown(300);
      $('.checkout__order-info').removeClass('checkout__steps-item--active').slideUp(300);
    }); // $('body').on('click', '.js-step-btn', function(e) {
    // 	e.preventDefault();
    // 	$('.checkout__user').toggleClass('checkout__steps-item--active');
    // 	$('.checkout__user').find('.checkout__steps-item--content').slideToggle(300);
    // 	$('.checkout__order-info').toggleClass('checkout__steps-item--active').slideToggle(300);
    // });
    //  more text
    ...
    $.fn.accordion = function () {
      ...
      $($accordionItemTitle).on('click', function (e) {
        ...
        } else if (!$currentItem.hasClass('addresses-accordion__item')) {
          closeAccordionItem();
        }
      });
    };

    if ($('.faq__accordion').length) {
    ...
    $('.gallery-item__link').on('click', function (e) {
      ...
      if (windowWidth > largeMobDesktop) {
        $.fancybox.open($('.gallery-item__link'), {
          touch: false,
          opts: {
            beforeShow: function beforeShow() {
              $('body').addClass('gallery-show');
    ...
    if ($('.gallery-slider').length) {
      if (windowWidth < largeMobDesktop) {
        var _$slider = $('.gallery-slider');

        var gallery = new Swiper(_$slider, {
          slidesPerView: 1,
          speed: 600,
    ...
    $('body').on('click', '.js-account-address-toggle', function () {
      $(this).toggleClass('active');
      $(this).next('.add__address-form').slideToggle(300);
    });
    $('[data-toggle="account__nav"]').click(function () {
      $('.account__nav-col').toggleClass('active');
      $('html').toggleClass('__fixed');
    });
    $('body').on('click', '.js-order-descr', function (e) {
      e.preventDefault();
      var content = $(this).closest('.table__row').find('.order__more').html();
      $('#modal__order').html(content);
      $.fancybox.open({
        src: '#modal__order',
        type: 'inline',
        opts: {
    ...
    $('body').on('click', '.js-blog__categories-trigger', function (e) {
      e.preventDefault();
      $(this).toggleClass('active');
      $(this).next('.blog__categories-list').slideToggle(300);
    }); // input file

    $(document).on('change', 'input.file-input', function () {
      var file = $(this).parent().find('input[type="file"]')[0].files[0];
      $(this).closest('.form__group-file').find('.file-label__text').text(file.name);
    });
    $('body').on('click', '.js-add-fg', function (e) {
      e.preventDefault();
      var $fgHolder = $(this).closest('.form__group-wrap');
      var $fg = $fgHolder.find('.fg:first').clone();
      var $btn = $(this).closest('.form__group-add');
      $fg.insertBefore($btn).find(".form__group-input").val("");
    }); // to top
    ...
    $(window).scroll(function () {
      ...
      // }
    }); // addresses accordion == start		

    $.fn.tabAccordion = function () {
      var accordion = $(this),
          $accordionItem = accordion.find('.tab__accordion-item'),
          $accordionActiveItem = accordion.find('.tab__accordion-item.active'),
          $accordionItemTitle = accordion.find('.tab__accordion-item--head'),
          $accordionItemContent = accordion.find('.tab__accordion-item--content');

      if ($accordionActiveItem.length) {
        $accordionActiveItem.find('.tab__accordion-item--content').show();
      }
      ...
      $($accordionItemTitle).on('click', function (e) {
        e.preventDefault();
        var $currentItem = $(this).closest('.tab__accordion-item');

        if (!$currentItem.hasClass('active')) {
          closeAccordionItem();
          $currentItem.addClass('active');
          $currentItem.find('.tab__accordion-item--content').slideDown(300);
        } else {
          closeAccordionItem();
        }
      });
    ...
    if ($('.addresses__accordion').length) {
      $('.addresses__accordion').accordion();
    }

    if ($('.tab__accordion').length) {
      $('.tab__accordion').tabAccordion();
    } // addresses accordion == end
    // map start ===================================================
    ...
    function initMap() {
      var locations = [],
          ...
          size = 3,
          cords = [],
          center = $('.map__nav-item.active').data('coord'),
          centerNum = center.split(',').map(Number),
          center2 = $('.map__nav-item.active').data('address'),
          map = undefined,
          self = {},
          geoResult = undefined,
      ...
      self._generateCoords = function (coords) {
        var coordNum = coords.split(',').map(Number);
        return coordNum;
      ...
      // 	console.log(addr);
      // });
      // self._geocodeR = function (address, callback) {				
      ...
      // 	});
      // }
      // let centerNum2 = self._geocodeR(center2);
      // console.log(center2);
      // console.log(centerNum2);


      $('.map__nav-item').on('click', function (e) {
        e.preventDefault();

        if (!$(this).hasClass('active') && $(this).attr('data-coord') != '') {
          $('.map__nav-item').removeClass('active');
          $(this).addClass('active');
          var centerNew = $(this).data('coord');

          var generateCords = self._generateCoords(centerNew);
      ...
      $('.js_btn_map').on('click', function (e) {
        e.preventDefault();
        ...
        if ($(this).attr('data-coord') != '') {
          ...
          map.setZoom(17);
        } else {// geocoder_kiev.geocode( {'address': $(this).attr('data-adress') }, function(results, status) {
          // 	map_kiev.setCenter(results[0].geometry.location);
          // 				map_kiev.setZoom(15);
          // 			});
        }

        var thisMapDataTab = $(this).closest('.addresses-accordion__item').find('.accordion__item--head').data('tab');
        var mapTab = $('.map__nav-item');
        var mapDataTabActive = $('.map__nav-item.active').data('tab');

        if (!(thisMapDataTab === mapDataTabActive)) {
          mapTab.removeClass('active');
          mapTab.filter('[data-tab=' + thisMapDataTab + ']').addClass('active');
        }
      });
      $('.js_btn_map').each(function (i, item) {
        cords.push($(item).closest('.tab__accordion-item').find('.tab__accordion-item--head').text());
        coord = $(item).data('coord');
        adress = $(item).data('adress');
        coordNum = coord.split(',').map(Number);
        cords.push(coordNum);
        cords.push(adress);
      });

      for (var _i = 0; _i < cords.length; _i += size) {
        locations.push(cords.slice(_i, _i + size));
      } // console.log(locations);


      map = new google.maps.Map(document.getElementById('map'), {
        zoom: 12,
        center: new google.maps.LatLng(centerNum[0], centerNum[1]),
        mapTypeId: google.maps.MapTypeId.ROADMAP
      });
      var marker, i;
      ...
      for (i = 0; i < locations.length; i++) {
        marker = new google.maps.Marker({
          position: new google.maps.LatLng(locations[i][1][0], locations[i][1][1]),
          map: map
        });
      ...
      // 		map: map
      // 	});	
      // 	google.maps.event.addListener(marker, 'click', (function (marker, latlng) {
      // 		return function () {
    ...
    if ($('#modal__discount').length) {
      var discount = getCookie("discount");
      var timeBeforeShow = parseInt($('#modal__discount').attr("data-time"));
      ...
      if (discount !== "no") {
        setTimeout(function () {
          ...
          document.cookie = "discount=no; expires=" + now.toUTCString() + "; path=/";
        }, timeBeforeShow);
      }
    } // modal discount end ======================
