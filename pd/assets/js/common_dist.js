...
(function ($) {
  ...
  $(function () {
    ...
    $('.header__auth-link').on('click', function (e) {
      e.preventDefault();
      var authorizationTab = $(this).data("authorization");
      var $authorizationTabClass = '.' + authorizationTab;
    ...
    $('[data="authorization-close"]').on('click', function () {
      $('html').removeClass('__show-overlay __show-header');
      if ($('.authorization').hasClass('__active')) $('.authorization').removeClass('__active');
    }); // input mask
    ...
    function initMasks() {
      $('.language-select').each(function () {
        ...
        var phoneInput = $(this).closest('.form__group-phone').find('input[type=tel]');
        var maskuse;

        if (country == 'ua') {
          maskuse = "+38 (000) 000 00 00";
        }

        if (country == 'ru') {
          maskuse = "+71 (000) 000 00 00";
        }

        if (country == 'kz') {
          maskuse = "+75 (000) 000 00 00";
        }
    ...
    function formatCountry(country) {
      ...
      var $country = $('<img src="' + baseUrl + '/' + country.element.value.toLowerCase() + '.png" class="country-img" />');
      return $country;
    }
    ...
    if ($(countrySelect).length) {
      $(countrySelect).each(function () {
        ...
        $(this).on('change', function (e) {
          ...
          if (country == 'ru') {
            maskuse = "+77 (000) 000 00 00";
          }

          if (country == 'kz') {
            maskuse = "+55 (000) 000 00 00";
          }
    ...
    $('.js-more-options').on('click', function (e) {
      e.preventDefault();
      var filterOptions = $(this).closest('.filter-tab__options-list').find('.js-filter-tab-more'),
    ...
    if ($('.reviews-slider').length) {
      var $slider = $(this).find('.reviews-slider');
      var productTabSlider = new Swiper($slider, {
        loop: true,
        slidesPerView: 3,
        speed: 600,
    ...
    $('body').on('click', '.js-review-more', function (e) {
      e.preventDefault();
      var reviews = $(this).closest('.reviews__item').find('.reviews__item-content').html();
      var reviewsHead = $(this).closest('.reviews__item').find('.reviews__item-header').html();
    ...
    $('body').on('click', '.js-modal-trigger', function (e) {
      e.preventDefault();
      var content = $(this).closest('.js-modal-item').find('.js-modal-content').html();
    ...
    function fancyImg() {
      ...
      if ($img.length) {
        $img.fancybox({
          ...
          baseTpl: '<div class="fancybox-container" role="dialog" tabindex="-1">' + '<div class="fancybox-bg"></div>' + '<div class="fancybox-toolbar">' + '{{buttons}}' + '</div>' + '<div class="fancybox-inner">' + '<div class="fancybox-navigation">' + '<button data-fancybox-prev title="{{PREV}}" class="fancybox-arrow fancybox-arrow--left"><i class="icon-chevron-thin-left"></i></button>' + '<button data-fancybox-next title="{{NEXT}}" class="fancybox-arrow fancybox-arrow--right"><i class="icon-chevron-thin-right"></i></button>' + '</div>' + '<div class="fancybox-stage"></div>' + '<div class="fancybox-caption-wrap">' + '<div class="fancybox-caption"></div>' + '</div>' + '</div>' + '</div>',
          btnTpl: {
    ...
    function interior() {
      var interiorImg = $('.product__interior-item.active').data('large');
    ...
    if ($('.product__images-slider').length) {
      var _promoSlider = new Swiper('.product__images-slider', {
        loop: false,
        speed: 650,
        roundLengths: true,
        setWrapperSize: true,
    ...
    $('body').on('click', '.toggle-switch button', function () {
      ...
      $('.toggle-block__btn').removeClass('active');
      $('.toggle-block__content').removeClass('not-visible'); // var view = $('#information').data('view');
      // var setShort = false;
      // if (view != undefined) {
      //     switch(view) {
      //         case 'short' : 
      ...
      $(this).addClass('js-link--active'); // нажали на просмотр в интерьере
        ...
        $('.short-view-out').each(function (index) {
          (function (that, i) {
            var t = setTimeout(function () {
              $(that).addClass('fadeOutRight').removeClass('fadeInRight fadeInLeft');
            }, 100 * i);
          })(this, index);
        });
      } else {
        ...
        items.forEach(function (el, index) {
          ...
          (function (that, i) {
            var t = setTimeout(function () {
              ...
              $(that).addClass('fadeInRight').removeClass('fadeOutRight fadeOutLeft');
            }, 100 * i);
          })(el, index);
          ...
          handler: function handler(direction) {
            if (direction == 'down') {
              if (!$(self).hasClass("show") && $(self).hasClass("up-animate")) {
                TweenMax.staggerFromTo($(self).find(".item-animate"), 0.5, {
                  opacity: 0,
                  y: 80
                }, {
                  opacity: 1,
              ...
              if (!$(self).hasClass("show") && $(self).hasClass("scale-animate")) {
                TweenMax.staggerFromTo($(self), 1.1, {
                  opacity: 0,
                  scale: 0.3
                }, {
                  opacity: 1,
              ...
              if (!$(self).hasClass("show") && $(self).hasClass("js-splitme")) {
                TweenMax.staggerFromTo($(self).find(".letter"), 0.3, {
                  y: 20,
                  opacity: 0
                }, {
                  y: 0,
